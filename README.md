...
4. When Allmon3 integration is enabled: makes `swp-data.json` inside the Allmon3 web root a **symlink** to that same canonical file (so there is only ever one real copy on disk, not two independent writes), and writes `swp-alerts.html` — a self-refreshing display page that fetches `swp-data.json` every 60 seconds
